...
namespace LootLabels {
    ...
    public class LootManager : MonoBehaviour {
        ...
        public Transform[] LootParents = new Transform[4];    //all dropped loot goes in this parent
        ...
        public void DropLoot(LootSource lootSource, Transform objectTransform) {
            ...
            if (LootParents[0].childCount > 0) {
                for (int i = 0; i < LootParents[0].childCount; i++) {
                    if (LootParents[0].GetChild(i).childCount == 0) {
                        //use this instead of instantiating a new origin
                        lootOrigin = LootParents[0].GetChild(i).gameObject;
                        lootOrigin.transform.SetPositionAndRotation(objectTransform.position, objectTransform.rotation);
                        break;
                    }

                    if (i == LootParents[0].childCount - 1) {
                        ...
                        lootOrigin.transform.SetParent(LootParents[0], false);
                    }
                }
            }
            else {
