	...
	public class RPGCharacterMovementController : SuperStateMachine
    {
        ...
        */
        ...
        private void Jump_EnterState()
        {
            ...
            canJump = false;
			rpgCharacterController.GetAngry();
        }
        ...
        private void ClimbLadder_SuperUpdate()
        {
            ...
            var ladderThreshold = 1.2f;

            if (moveInput.y > 0f) {
                var ladder = rpgCharacterController.ladder;
                var ladderTop = new Vector3(transform.position.x, ladder.bounds.max.y - ladderThreshold, transform.position.z);
                ...
                var ladder = rpgCharacterController.ladder;
                var ladderBottom = new Vector3(transform.position.x, ladder.bounds.min.y + ladderThreshold, transform.position.z);
