...
namespace DunGen
{
	...
	public sealed class DoorwayPairFinder
	{
		...
		private bool CanTilesConnect(Tile tileA, Tile tileB, Doorway doorwayA, Doorway doorwayB)
		{
			foreach(var rule in CustomConnectionRules.OrderByDescending(r => r.Priority))
			{
				var result = rule.Delegate(tileA, tileB, doorwayA, doorwayB);
			...
			return DoorwaySocket.CanSocketsConnect(doorwayA.Socket, doorwayB.Socket) && DungeonFlow.CanTilesConnect(tileA, tileB);
		}
		...
		private bool IsValidDoorwayPairing(DoorwayProxy a, DoorwayProxy b, TileProxy previousTile, TileProxy nextTile, ref float weight)
		{
			...
			if (!CanTilesConnect(PreviousTile.PrefabTile, nextTile.PrefabTile, a.DoorwayComponent, b.DoorwayComponent))
				return false;
